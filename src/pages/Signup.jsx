...
const EditProfileModal = ({ user = {}, setUser, closeModal, onSave }) => {
  ...
  // mount time to ignore immediate taps/clicks on touch devices
  ...
  // Backdrop click: only close when user actually clicks/taps the backdrop
  // and not on the same tick as mount. This prevents immediate close on touch devices.
  const handleBackdropClick = (e) => {
    const elapsed = Date.now() - (mountTimeRef.current || 0);
    if (elapsed < 300) return; // increased delay to handle slower touch event timing on some devices
