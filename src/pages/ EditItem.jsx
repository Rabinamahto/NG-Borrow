...
const EditItem = () => {
            ...
            try {
                if (db) {
                    ...
                    if (snap.exists()) {
                        ...
                        const isMine = currentUid && data.ownerId && currentUid === data.ownerId;

                        if (!isMine) {
                            setError('You are not the owner of this item or do not have permission to edit it.');
                            setLoading(false);
                            return;
                        }

                        const foundItem = { id: snap.id, ...data };
                        setItem(foundItem);
    ...
    const handleSave = async (e) => {
        ...
        // Try saving to Firestore if item exists there
        try {
            if (db && item && item.ownerId) {
